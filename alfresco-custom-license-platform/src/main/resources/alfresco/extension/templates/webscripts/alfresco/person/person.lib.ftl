...
<#local LICENSE_TYPE = "license_type">
<#local LICENSE_LIMIT = "license_limit">
<#local LICENSE_VALIDITY_SPAN = "license_span_in_days">
<#local ATTACHED_LICENSE = "license_key">

<#escape x as jsonUtils.encodeJSONString(x)>
	"url": "${url.serviceContext + "/api/custom/people/" + p.userName}",
	"userName": "${p.userName}",
    ...
    "license_type": "<#if p[LICENSE_TYPE]??>${p[LICENSE_TYPE]?js_string}</#if>",
    "license_limit": "<#if p[LICENSE_LIMIT]??>${p[LICENSE_LIMIT]?js_string}</#if>",
    "license_span_in_days": "<#if p[LICENSE_VALIDITY_SPAN]??>${p[LICENSE_VALIDITY_SPAN]?js_string}</#if>",
    "license_key": "<#if p[ATTACHED_LICENSE]??>${p[ATTACHED_LICENSE]?js_string}</#if>"
</#macro>
