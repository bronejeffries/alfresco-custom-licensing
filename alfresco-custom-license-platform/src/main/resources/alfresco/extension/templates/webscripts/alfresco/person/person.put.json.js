function main()
{
   ...
   if ((!json.isNull("organisation")) && (!immutableProperties.hasOwnProperty((qname + "organization").toString())))
   {
    ...
    //save the extra properties
   var extra_properties = ["license_type","license_limit","license_span_in_days","license_key"]
   for (var index=0; index<extra_properties.length; index++)
   {
     if ((!json.isNull(extra_properties[index])))
     {
       person.properties[extra_properties[index]] = json.get(extra_properties[index]);
     }
   }
